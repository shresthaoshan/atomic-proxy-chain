...
const forward = async (request, response, handlerOpts) => new Promise(async (resolve, reject) => {
    ...
    response.once('close', () => {
        const { requestId, customTag, id: connectionId, server, } = handlerOpts;
        const { bytesWritten, bytesRead } = (0, count_target_bytes_1.getTargetStats)(request.socket);
        server.emit('requestFinished', {
            ...
            customTag,
            stats: {
                srcRxBytes: request.socket.bytesRead,
                srcTxBytes: request.socket.bytesWritten,
